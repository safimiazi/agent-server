...
const saveClientData = async (data) => {
  try {
    ...
    if (!data || Object.keys(data).length === 0)
      return { message: "Something wrong: No data provided" }; // Check if data is provided
    ...
    if (existingUser) {
      return {
        message:
          "User with the provided uniqueId already exists in the database",
      }; // User already exists
    }
    ...
    if (existingCustomer) {
      return {
        message:
          "Customer with the provided customerName already exists in the database",
...
const getingAllregiserUser = async () => {
  try {
    return;
    console.log("access geting user register");
...
const getClientDataList = async (searchQuery, page = 1, pageSize = 50) => {
  try {
    ...
    const clientListData = await Customer.find(query)
      .skip(skip)
      .limit(pageSize); // with quiery data and pagination
    ...
    const finalliyValue =
      clientListData.length > 0 ? clientListData : withOutQuery;
...
const editeClientData = async (clientId, newClientData) => {
  try {
    ...
    if (
      clientId === "undefined" ||
      !clientId ||
      Object.entries(newClientData).length < 1
    ) {
